...
def trójki(T):
    ...
    for index_1 in range(len(T)-3):
        ...
        if czy_1_to_NWD(T[index_1],T[index_2],T[index_3]) == True:
            ...
            if index_3<(len(T)-1):
                if czy_1_to_NWD(T[index_1], T[index_2], T[index_3+1]) == True:
                    liczba_trójek += 1
                    trójka = [T[index_1], T[index_2], T[index_3+1]]
                    lista_trójek.append(trójka)

            if czy_1_to_NWD(T[index_1], T[index_2 + 1], T[index_3 + 1]) == True:
                liczba_trójek += 1
                trójka = [T[index_1], T[index_2+1], T[index_3+1]]
                lista_trójek.append(trójka)

            if index_3<(len(T)-2):
                if czy_1_to_NWD(T[index_1], T[index_2 + 1], T[index_3 + 2]) == True:
                    liczba_trójek += 1
                    trójka = [T[index_1], T[index_2+1], T[index_3+2]]
                    lista_trójek.append(trójka)
